    ...
    inline_keyboard=[
        [
            InlineKeyboardButton(
                text="🎶 Let the party begin!", callback_data=PostCallback(notification=True).pack()
            )
        ],
        [
            InlineKeyboardButton(
                text="🔕 Post this silently", callback_data=PostCallback(notification=False).pack()
            )
        ],
        [
            InlineKeyboardButton(
                text="🎵 Replace file", callback_data=EditCallback(what="song").pack()
            )
        ],
        [
            InlineKeyboardButton(
                text="✏️ Edit YouTube link", callback_data=EditCallback(what="yt_music").pack()
            )
        ],
        [
            InlineKeyboardButton(
                text="✏️ Edit Yandex link", callback_data=EditCallback(what="yandex").pack()
            )
        ],
        [
            InlineKeyboardButton(
                text="✏️ Edit SoundCloud link", callback_data=EditCallback(what="scloud").pack()
            )
        ],
        [
            InlineKeyboardButton(
                text="❌ Cancel", callback_data=ActionCallback(action="delete").pack()
            )
        ],
    ]
)
...
def extract_links(links: dict[str, dict[str, str]]) -> dict[str, str]:
    return {
        ...
        for src, dest in zip(
            ("spotify", "youtubeMusic", "yandex", "soundcloud"), FRIENDLY_NAMES.keys()
        )
    }
...
async def audio_first_seen(event: Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="⏩ Continue anyway", callback_data=ActionCallback(action="retry").pack()
                )
            ]
        ]
    )
    ...
    if not ok:
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="🔁 Try again", callback_data=ActionCallback(action="retry").pack()
                    )
                ]
            ]
        )
